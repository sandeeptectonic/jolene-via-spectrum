async function getCp(waybill) {
  try {
    const url = 'https://jolene.clickpost.in/return-exchange?waybill='+ waybill+'&security_key=';
    const response_cp = await fetch('https://jolene.clickpost.in/api/v1/track-order?null=null&waybill='+waybill);
    const return_cp_data = await response_cp.json();
    const cp_id = return_cp_data.result.cp_id;
    const response_return = await fetch('https://www.clickpost.in/api/v1/shipment-security-key?waybill=' + waybill + '&cp_id=' + cp_id + '&key=5cf9aba3-8774-40e4-8db1-898e42e90a26');
    const return_data = await response_return.json();
    if(return_data){
      window.open(url+return_data.result.security_key);
    } 
...
var acc = document.getElementsByClassName("prd_info_title");
var i;
...
if ($('.tabs_title_prd li.details_tab ').hasClass('active_tab')) {
  $('.prd_row_data .details_data_tab').addClass('active_tab_data');
}
...
function updateRange() {
  ...
  if (maxRange - minRange < 100) {
    ...
    const newMinValue = Math.max(0, middleValue - 50);
    const newMaxValue = Math.min(rangeInput[1].max, middleValue + 50);
...
function addToCart(variantId, qty) {
  $('body').addClass('LoadingCart');
  setTimeout(function () {
    $('body').removeClass('LoadingCart');
  }, 2200);
  $.ajax({
    url: '/cart/add',
    type: 'POST',
    ...
    success: function (response) {
      $.ajax({
        ...
        success: function (carthtml) {
          ...
          $('body').addClass('overflow-cart-hidden overflow-hidden');
 ...
 var element = document.getElementsByClassName('collection-category-block');

    // Check if the element has any child nodes
    if (element && !element.hasChildNodes('div')) {
        // If it has no children, set its display style to 'none'
